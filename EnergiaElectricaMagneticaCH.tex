\chapter{Energía eléctrica y magnética}

\section{Energía electrostática.}


El trabajo necesario para conformar una distribución de carga se almacena como energía potencial en el campo eléctrico, porque si se liberan las cargas, este trabajo se podría recuperar.


\subsection{Trabajo necesario para conformar una distribución de cargas puntuales.}


Supóngase que se construye un sistema de $n$ cargas puntuales, trayendo cada carga desde el infinito hasta su posición en la distribución, como se ilustra en la figura \ref{g_DisCar}.

\begin{figure}[hb]
	\centering
		\includegraphics{7g1.1}
	\caption{Distribución de cargas en un universo de permitividad $\epsilon$.}
	\label{g_DisCar}
\end{figure}
...
$\phi_i$ es el potencial en $i$ debido a las $i-1$ cargas existentes. El trabajo total para formar la distribución será:
\begin{equation}
...
De la ecuación \eqref{ec_Trab1}, la primera suma representa la suma de los términos de una matriz triangular inferior con $i$ creciendo horizontalmente y $j$ creciendo verticalmente. Cada término sería $\frac{q_i\,q_j}{4\pi\epsilon r_{ij}}$, la segunda es la suma de los términos de la matriz triangular superior que completa la matriz cuadrada para el arreglo.
...
Tomando la definición del potencial electrostático, se puede escribir así:
\begin{equation}
\begin{gathered}
...
siendo $\phi$ el potencial de la distribución. Como por definición la energía electrostática es el trabajo realizado necesario para conformar la distribución de carga, entonces:
\begin{equation}
...
Por la condición de regularidad en infinito para $\phi$ y $\mathbf{D}$, si el volumen $V$ tiende a ser el universo, entonces $S$ sería la superficie límite del universo en donde el producto $\phi\,\mathbf{D}$ es un vector que decrece a razón de $\frac{1}{r^3}$, mientras el diferencial de superficie crece al ritmo de $r^2$. Se sigue que la integral en $S$ es nula:
\begin{equation}
...
Nótese que $All$ denota todo el espacio.
...
\begin{figure}[hb]
	...
	\caption{Circuito eléctrico de un condensador cargándose.}
	\label{g_ConCar}
\end{figure}

La fuente de voltaje $V_{DC}$ realiza un trabajo para depositar carga positiva en $b$ mientras retira carga de $a$. El trabajo realizado entre el momento en que se conecta el condensador descargado y el momento en que está completamente cargado es $W=q V$, donde $q$ es la carga suministrada por la fuente y $V$ la diferencia de potencial instantanea que varía desde $0 V$ hasta $V_{DC}$. El trabajo diferencial realizado cuando se ha suministrado una carga $dq$ es $dW=dq V$. Dado que la capacitancia es constante $C=Q/\Delta \phi$, la carga diferencial suministrada por la fueste se puede escribir en términos del potencial instantáneo de la fuente así: $dq=CdV$. En consecuencia el trabajo realizado por la fuente es:
\begin{equation}
W=\int\limits_{0}^{V_{DC}} C V dV=\frac{C V_{DC}^2}{2}.
\end{equation}

Rápidamente se puede demostrar que:
\begin{equation}
...
\subsection{Energía de ligadura de un átomo.}


Si logramos simplificar el átomo y considerarlo como una nube de electrones esférica, uniformemente cargada con una carga puntual (núcleo) en el origen, como se observa en la gráfica \ref{g_SimAt}, entonces para encontrar la energía, por superposición, primero se encontrará la energía de la nube, luego se añadirá el núcleo.

\begin{figure}[hb]
	\centering
		\includegraphics{7g3.3}
	\caption{Simplificación del átomo. La nube de electrones y núcleo con cargas $-Q$ y $Q$ respectivamente.}
	\label{g_SimAt}
\end{figure}
...
Luego, la energía almacenada en la nube de electrones es:
\begin{equation}
\begin{gathered}
...
Luego se determina el trabajo necesario para colocar el núcleo en el centro del volumen:
\begin{equation}
\begin{gathered}
...
La energía total es $U=U_-+U_+$, luego:
\begin{equation}
	U=\frac{-9Q^2}{40\pi\epsilon_0R}
\end{equation}

Y ¡obsérvese que esta energía es negativa!


\subsection{Fuerzas sobre conductores.}


Imagínese una porción de un conductor como se muestra en la figura \ref{g_PorCon}.

\begin{figure}[hb]
	\centering
		\includegraphics{7g4.4}
	\caption{Porción de un conductor.}
	\label{g_PorCon}
\end{figure}
...
Si el conductor está en condiciones electrostáticas, el campo en su interior es nulo:
\begin{equation}
...
\subsection{Relación fuerza energía.}


Se sabe que la energía electrostática es el trabajo necesario para formar la distribución de carga. Entonces:
\begin{equation}
...
Aquí $\mathbf{F}$ es la fuerza generada por el campo.
...
\begin{figure}[hb]
	...
	\caption{Condensador de placas paralelas de área $A$.}
	\label{g_FuerConPla}
\end{figure}
...
La ecuación \eqref{ec_FuerAtrac} da a entender que es una fuerza de atracción. Por el lado de la energía:
...
La última expresión de la ecuación \eqref{ec_FuerPla} se interpreta como la fuerza sobre la placa superior.

El signo negativo en $	\mathbf{F}=-\nabla U$ expresa la tendencia natural que tiene el sistema eléctrico a tratar de disminuir su energía potencial, luego sí se presenta confusión en el signo de la fuerza; éste de ser elegido según el criterio anterior.
...
\end{equation}
Se sigue el siguiente desarrollo, donde se considera que $U_B$ es la energía magnética:
...
La ecuación \ref{ec_trabajom6} es la energía magnética almacenada en un circuito que tiene la corriente de establecimiento $I$ y que tiene por autoinductancia $L$. Existen otras dos expresiones equivalentes a \ref{ec_trabajom6}, si se hacen los reemplazos convenientes al utilizar la relación entre flujo magnético enlazado y corriente.

\begin{equation}\label{ec_flujom1}
	\Phi_B\,=LI\quad\text{Donde L es la autoinductancia}
\end{equation}

De allí se pueden obtener las siguientes igualdades:
\begin{equation}\label{ec_flujom2}
	U_B=\frac{LI^2}{2}=\frac{I\Phi_B}{2}
\end{equation}

\subsection{Energía magnética de una distribución volumétrica de corriente}
...
\end{equation}
Aprovechando que todo campo magnético es el rotacional del potencial vectorial magnético ($\bm B=\bm\nabla\times\bm A$) y haciendo uso del teorema del rotacional de Stokes
\begin{equation}\label{ec_flujom4}
	\Phi_B=\iint\limits_{S}\bm{\nabla}\times\bm{A}\cdot d\bm S = \oint\limits_{C}\bm{A}\cdot d\bm{r}
\end{equation}

Y para la energía magnética:
\begin{equation}\label{ec_flujom6}
	\boxed{U_B=\frac{I}{2}\oint\limits_{C}\bm{A}\cdot\,d\bm{r}}
\end{equation}

Los elementos diferenciales de corriente son equivalentes entre sí, tal como se ilustra en \ref{ec_flujom7}
\begin{equation}\label{ec_flujom7}
 Id\bm{r}=\bm{I}dr=\bm{K}dS=\bm{J}dv
\end{equation}

La versión de \ref{ec_flujom6} para el caso de una distribución volumétrica de corriente es  así:
\begin{equation}\label{ec_flujom8}
 U_B=\frac{1}{2} \iiint\limits_{V}\bm{A}\cdot\bm{J}dv
\end{equation}
Ahora, considerando la ley de Ampere para el integrando de \ref{ec_flujom8} se puede llegar a la siguiente expresión:
...
La estrategia es la siguiente: aprovechando que el sistema tiene las suficientes simetrías, se puede calcular tanto $\bm B$ como $\bm H$ en todo punto. Con estos resultados se puede averiguar cómo es la energía magnética almacenada en una porción de cable de longitud $h$. Finalmente, teniendo en cuenta la ecuación \eqref{ec_flujom10} que relaciona la energía magnética almacenada en una inductacia y su autoinductancia, se puede obtener la variable pedida.
...
En este todos los medios son similares al vacío, luego $\bm{B}=\mu_0\bm{H}$. La energía magnética se puede escribir como:
\begin{equation}\label{ec_4}
	U_B=\iiint\limits_{V}\frac{\mu_0\,H^2}{2}dv
\end{equation}

Utilizando esta ecuación junto con los resultados anteriores, se llega luego de integrar a:
...
La inductancia se puede dividir en inductancia interna e inductancia externa. La primera mide la capacidad de almacenar energía en forma de campo magnético en el interior de los conductores que forman el inductor. La segunda hace lo propio pero con el la energía almacenada fuera de los conductores.

Para el caso del cable coaxial, la inductancia externa mide la capacidad de almacenar energía en forma de campo magnético en la región exterior a ambos conductores, es decir en $a<\rho<b$.
\begin{equation}\label{ec_6}
	L_{ext}=\frac{\mu_0\,h}{2\pi}\ln\left(\frac{b}{a}\right).
\end{equation}

La inductancia interna del conductor interno es $\displaystyle\frac{\mu_0}{8\pi}$, y no depende del radio de éste. Algo que podría ser sospechoso a primera vista pero que no guarda mayores dificultades si se considera que a mayor inductancia, mayor capacidad de almacenar energía en forma de campo magnético. Ello implica que, para propósitos de comparación, dos inductores con autoinductancias $L_1$ y $L_2$ que satisfagan la relación $L_1<L_2$; a igual corriente la energía magnética almacenada por el primero es mayor que la correspondiente almacenada por el segundo (siguiendo a \ref{ec_trabajom6}).

\section{Potencia electromagnética}

Considérese un volumen cargado, sometido a la presencia de los campos $\bm{E}$ y $\bm{B}$.
...
\end{equation}
Dado que el diferencial de trabajo debido al movimiento diferencial $d\bm{r}$ es $dW=\bm{F}\cdot d\bm{r}$, se puede expresar el trabajo diferencial realizado por los campos cuando un agente externo mueve el $dq$ un diferencial de línea $d\bm{r}$ así:
...
En \ref{ec_7} debe tenerse en cuenta la relación existente entre la densidad volumetrica de corriente libre y la velocidad $\bm{J}_f=\rho_f\bm{V}$, así como la propiedad del producto triple $\bm{A}\cdot\bm{B}\times\bm{C}=\bm{A}\times\bm{B}\cdot\bm{C}$.

Finalmente se obtiene \ref{ec_8}, donde se observa que el campo magnético \emph{NO HACE TRABAJO} sobre las cargas en movimiento.
\begin{equation}
 \label{ec_8}
...
La interpretación que se realiza para la ecuación \ref{ec_potencia} es la siguiente:
\begin{equation}\label{ec_potencia2}
 \bm{J}_f\cdot\bm{E}=\left\{
 \begin{array}{cl}
 >0 \ ; &P\text{ \ se disipada en forma de calor.}\\
 <0 \ ; &P\text{ \ es suministrada y se transforma en potencia electromagnética.}
 \end{array}\right.
\end{equation}

\subsection{Teorema de Poynting}

Independiente de si la potencia en \ref{ec_potencia} es mayor o menor que cero, se puede escribir su integrando con ayuda de la ley de Ampere para $\bm H$ así:
...
\begin{split}
	\mu_E\equiv&\frac{\bm{E}\cdot\bm{D}}{2}\quad\text{Densidad de energía eléctrica}.\\
	\mu_B\equiv&\frac{\bm{H}\cdot\bm{B}}{2}\quad\text{Densidad de energía magnética}.\\
	\bm{\mathbb S}\equiv&\bm{E}\times\bm{H}\quad\text{Vector de Poynting}\\
	\mu_{EB}\equiv&\mu_E+\mu_B\quad\text{Densidad de energía electromagnética}.\\
	U_{EB}\equiv&\iiint\limits_{V}\mu_{EB}dV\quad\text{Emergía electromagnética en}V.
\end{split}
\end{equation}
...
\subsection{Ilustración}
El siguiente ejercicio muestra la perdida de la energía en el sistema.
...
Entonces, $?$Que pasó con la energía magnética$?$
...
\subsection{6.3.3 Disipación de potencia en una resistencia.}
...
Comenzemos con el análisis deseado:
...
\end{equation}
	...
	Ahora ulitizamos \eqref{ec_potencia4} para encontrar el valor de $\bm{S}$; así:
...
\begin{aligned}
\oiint\limits_{S}\bm{S}\cdot\,d\bm{s}=\frac{-E_0\,I_0}{2\phi\,R^2}\iint\limits_{S'}\rho\cdot\,d\bm{s}\hat{\rho}\quad\text{Solo se evalúa la frontera latera ya}\\
...
\begin{equation}\label{ec_17}
	P=E_0\,I_0\,L
\end{equation}
	Ahora, a modo de prueba, vamos a partir de la ecuación de potencia trabajada en los circuitos eléctricos:
...
\end{equation}
	Pero $J_f$ se define como la corriente por unidad de área, es decir $\frac{I_0}{\pi\,R^2}$, de allí que:
...
\chapter{Campo eléctrico de movimiento}

En este capítulo, vamos a estudiar lo que acontece cuando se cambia el sistema de referencia sobre un fenómeno electromagnético.
%Echar mas carreta
%Echar mas carreta
%Echar mas carreta

%Todo estudiante de teoría electromagnétiga sabe que:
%\begin{equation}
...
Para un punto , se tiene que Z=Z' y Y=Y', además que en $t=0$ los centros de coordenadas de ambos sistemas coinciden.
...
$\bullet$ Supongamos $\bm{F}_EB=q[\bm{E}+\bm{V}\times\bm{B}]$ como un invariante Galileano\footnote{Derivado del principio de la relatividad, según el cual las leyes fundamentales de la física son las mismas en todos los sistemas de referencia inerciales.}.

\section{7.1. Ilustración.}

Supongamos un campo magnético en $\hat{-x}$, y una carga que se mueve con velocidad $V_0\hat{y}$, como muestra la figura.
%La figurita "animada" de la carga
%La figurita "animada" de la carga
%La figurita "animada" de la carga
Para la carga, se tiene la siguiente fuerza debida a los efectos magnéticos:
\begin{equation}\label{ec_22}
	\bm{F}=qV_0\,B_0\hat{z}
\end{equation}

Ahora tomemos un marco de referencia diferente, así:
%La figurita "animada" de la carga2
%La figurita "animada" de la carga2
%La figurita "animada" de la carga2
En este caso, el sistema de coordenadas se mueve con la carga, y por tanto $\bm{V}=0$. Esto trae como consecuencia, que la fuerza debida al campo magnético sea 0; pero como ya se había mencionado, los efectos de fuerza sobre la carga no pueden cambiar entre los diferentes sistemas de referencia inerciales. Es por esto, que se debe asociar una fuerza $\bm{F'}$ a la carga, que responda a la fuerza debida al campo magnético.
\begin{equation}\label{ec_23}
	\bm{F'}=\bm{F}_B
\end{equation}
Suponemos un campo magnético $\bm{E'}$:
...
Este es el campo eléctrico de movimiento.

\subsection{7.1.1. Efecto Hall.}

El efecto hall, es el fenómeno que ocurre en un conductor por el que circula una corriente, en presencia de un campo magnético perpendicular al movimiento de las cargas.

Al pasar las cargas, estas se ven afectadas por el campo magnético, y se van acumulando de uno de los lados del conductor; a medida que se acumulan cargas, se inducen otras de signo opuesto en el otro extremo del conductor produciendo finalmente un campo eléctrico en la zona afectada por el campo magnérico.
Cuando el fenómeno se ha estabilizado, las cargas que pasan por la zona afectada por el campo magnético, no sienten fuerza alguna, ya que la fuerza magnética es compensada con la fuerza eléctrica.
%Figuritas del efeto hall
%Figuritas del efeto hall
%Figuritas del efeto hall
%Figuritas del efeto hall

De lo anterior se obtiene lo siguiente, analizando el fenómeno desde un sistema de referencia estático:
...
Calculando el campo eléctrico total:
...
%?Nombre del capitulo?
%?Nombre del capitulo?
\chapter{Campos electromagnéticos y la ecuación de onda}

\section{Corriente de desplazamiento}
Cuando recién se planteó, la ley de Ampere arrojaba resultados ilógicos, al abordar ciertos problemas.
...
Pero ahora, si aplicamos la misma ecuación en otra linea de integración, así:
...
Tiempo después llegó el británico James Clerk Maxwell e hizo el siguiente análisis sobre la ley de Ampere:
...
Suponiendo que la corriente es continua aún en el condensador, y continuando con el análisis:
...
Como la corriente debe ser continua, se sabe que $I_d=I_0$, pero a continuación se hará una breve demostración de ello, y de otros valores importantes.
...
Y finalmente para el cálculo de $\bm{S}$
...
\section{Los campos electromagnéticos y la ecuación de onda.}
...
Claramente, la ecuación anterior satisface la ecuación de onda, para la cual
...
''La luz es un caso particular de las ondas electromagnéticas''

\subsection{Solución de la ecuación de onda.}

Partiendo de la ecuación \eqref{ec_ondas1}, se estudia el caso de una onda con simetrías en $\hat{x}$ y en $\hat{y}$, es decir:
\begin{equation}\label{ec_40}
	\bm{E}=\bm{E}_x\,(z,t)+\bm{E}_y\,(z,t)+\bm{E}_z\,(z,t)
\end{equation}
Ademas, se supone que las ecuaciones que describen el comportamiento del campo son independientes entre sí:
...
Finalmente, el campo será la suma de todas las posibles soluciones:
\begin{equation}\label{ec_46}
	E(z,t)=\sum\limits_K{(A_z\,e^ikz+B_z\,e^-ikz\,)(A_t\,e^i\omega\,z+B_z\,e^-iwz\,)}
\end{equation}
Al desarrollar los productos de los paréntesis, se toman solo dos de las soluciones\footnote{Convención para el electromagnetismo}
\begin{equation}\label{ec_47}
	E(z,t)=\sum\limits_K{(A\,e^i(kz+\omega\,t)\,)+(B\,e^i(kz-\omega\,t)\,)}
\end{equation}

\subsection{Solución fasorial}
...
\end{equation}
Aplicando la ecuación \eqref{ec_ondas1} a la expresión anterior, se obtiene el siguiente resultado:
...
Se definen otros valores importantes:\\
	$\ast$ La fase del campo eléctrico: $\varphi_E\,=Kz+\omega\,t+\varphi_A$\\
	$\ast$ La velocidad de fase\footnote{La velocidad de un punto con fase constante}: $V_ph\,=\frac{dz}{dt}=\frac{-\omega\,}{K}<0$\\
	$\ast$ La constante de propagación ''K''

\subsection{Ilustraciones}

Vamos a analizar en esta parte del documento, la propagación de una onda electromagnética plana\footnote{Onda de frecuencia constante, cuyos frentes de onda son planos paralelos de amplitud constante}, en un medio conductor.

Para el analisis se tiene la siguiente solución de onda:
...
Reemplazando \eqref{ec_fasor0} en la ecuación anterior, se obtiene:
...
Y finalmente se obtiene la siguiente solución:
...
Se introduce el valor Q, como el factor de calidad, definido así:
